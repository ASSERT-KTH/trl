...
def mask_tool_response_tokens(
    ...
    """
    Masks tool response tokens from loss computation in GRPO training.
    
    Tool responses are generated by external tools during agent interactions and should not 
    contribute to the language model's training loss. This function identifies and masks these 
    tokens while preserving the conversational context for attention computation.
    
    Args:
        completion_ids (`torch.Tensor`): 
            Token IDs for model completions of shape (batch_size, sequence_length).
        completion_mask (`torch.Tensor`): 
            Binary mask indicating valid completion tokens of shape (batch_size, sequence_length).
            Values of 1 indicate tokens that contribute to loss, 0 indicates padding/ignored tokens.
        processing_class (`PreTrainedTokenizerBase`): 
            The tokenizer used for encoding. Different tokenizers use different tool response patterns.
            
    Returns:
        `torch.Tensor`: Updated completion mask with tool response tokens set to 0.
        
    Supported Patterns:
        - **Qwen3**: `<tool_response>content</tool_response>` (single-token tags)
        - **Qwen2**: `<|im_start|>tool\ncontent\n<|im_end|>` (multi-token sequence)  
        - **Llama-3**: `<|start_header_id|>ipython\ncontent\n<|eot_id|>` (multi-token sequence)
        - **Mistral**: `<tool>content</tool>` (multi-token tags)
        
    Example:
        ```python
        >>> tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen3-0.6B")
        >>> text = "I'll help<tool_response>file1.txt\nfile2.txt</tool_response>Done"
        >>> tokens = tokenizer.encode(text, add_special_tokens=False)
        >>> completion_ids = torch.tensor([tokens])
        >>> completion_mask = torch.ones_like(completion_ids)
        >>> masked = mask_tool_response_tokens(completion_ids, completion_mask, tokenizer)
        >>> print(f"Masked {(completion_mask.sum() - masked.sum()).item()} tool response tokens")
        Masked 12 tool response tokens
        ```
        
        ```txt
        Token-by-Token Masking Visualization:
        
        Token ID   Text                 Original Mask    Final Mask
        --------------------------------------------------------
        ...
        3542        files               1                1        ✓ Keep
        13         .                    1                1        ✓ Keep
        151665     <tool_response>      1                0        ✗ Mask start tag
        1192       file                 1                0        ✗ Mask content
        16         1                    1                0        ✗ Mask content
        3909       .txt                 1                0        ✗ Mask content
        198        \n                   1                0        ✗ Mask content
        1192       file                 1                0        ✗ Mask content
        17         2                    1                0        ✗ Mask content
        7197       .py                  1                0        ✗ Mask content
        198        \n                   1                0        ✗ Mask content
        54675      README               1                0        ✗ Mask content
        21324      .md                  1                0        ✗ Mask content
        151666     </tool_response>     1                0        ✗ Mask end tag
        8420       Here                 1                1        ✓ Keep
        525         are                 1                1        ✓ Keep
        279         the                 1                1        ✓ Keep
        3542        files               1                1        ✓ Keep
        13         .                    1                1        ✓ Keep
        ...        ...                  ...              ...      (continues for multi-turn)
        
        Result: 12/23 tokens masked (tool response content excluded from loss)
        ```
        
    Note:
        This function operates on the loss mask level, not attention. Tool response tokens remain 
        visible during forward passes to maintain conversational coherence but contribute zero 
        gradient during backpropagation.
    """
    # Early return if no completion tokens
    ...
    # Define tool response patterns supported across model families
    tool_patterns = [
    ...
    # Find which patterns this tokenizer supports
    supported_patterns = []
    for start_pattern, end_pattern in tool_patterns:
        try:
            ...
            if start_tokens and end_tokens:
                supported_patterns.append((start_tokens, end_tokens))
        except Exception:
            continue
    ...
    # Create mask for tool response tokens (start with all ones = keep all tokens)
    tool_mask = torch.ones_like(completion_mask, dtype=completion_mask.dtype, device=device)
    
    # Process each sequence in the batch
    for batch_idx in range(batch_size):
        tokens = completion_ids[batch_idx].tolist()
        
        # Apply each supported pattern
        for start_tokens, end_tokens in supported_patterns:
            i = 0
            while i < len(tokens):
                # Look for start pattern
                if _tokens_match_at_position(tokens, start_tokens, i):
                    # Found start - mask from here
                    start_pos = i
                    i += len(start_tokens)
                    
                    # Mask start tokens
                    for pos in range(start_pos, i):
                        if pos < seq_len:
                            tool_mask[batch_idx, pos] = 0
                    
                    # Continue until we find end pattern
                    while i < len(tokens):
                        if _tokens_match_at_position(tokens, end_tokens, i):
                            # Found end pattern - mask it and stop
                            for pos in range(i, i + len(end_tokens)):
                                if pos < seq_len:
                                    tool_mask[batch_idx, pos] = 0
                            i += len(end_tokens)
                            break
...
def _tokens_match_at_position(tokens: list[int], pattern: list[int], position: int) -> bool:
    """Helper function to check if token pattern matches at given position."""
    if position + len(pattern) > len(tokens):
        return False
    return tokens[position:position + len(pattern)] == pattern
